...
from re import sub
from utilities import searchWoK
...
def returnhtml():
    ...
    try:
        os.makedirs(os.path.join(os.getcwd(),'resources', 'msLoad'))
    except OSError:
        if not os.path.isdir(os.path.join(os.getcwd(),'resources', 'msLoad')):
            raise
    try:
        os.makedirs(os.path.join(os.getcwd(),'resources', 'rawICSD'))
    except OSError:
        if not os.path.isdir(os.path.join(os.getcwd(),'resources', 'rawICSD')):
            raise

    prevload = os.listdir(os.path.join(os.getcwd(),'resources', 'msLoad'))
    rawicsd = os.listdir(os.path.join(os.getcwd(),'resources', 'rawICSD'))

    with open(os.path.join(os.getcwd(),'web', 'ms.html')) as f:
        rawhtml = f.read()
    with open(os.path.join(os.getcwd(),'web', 'JS', 'jquery-1.11.3.min.js')) as f:
        jqueryjs = f.read()
    with open(os.path.join(os.getcwd(),'web', 'ms.js')) as f:
        msjs = f.read()
...
def mainapp():
    ...
    if searchtype == 'load' or searchtype == 'sort' or searchtype == 'save':
        ...
        elif searchtype == 'save':
            data = loads(escape(d['text']))

            searchWoK.handlehtmlsearch_save(data)
        ...
        if searchtype == 'mp':
            ...
            if searchtype == 'wok':
                ...
                prevcsv = next(os.walk(os.path.join(os.getcwd(), 'results', 'materialsSearchCSV-WC')))
